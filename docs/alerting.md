...
```promql
sum by (from_domain) (increase(resend_email_events_total{event_type="email.failed"}[5m])) > 0
```
...
```promql
sum by (from_domain, to_domain) (increase(resend_email_events_total{event_type="email.bounced"}[5m])) > 0
```

Narrow it to a specific recipient provider:

```promql
sum(increase(resend_email_events_total{event_type="email.bounced",to_domain="outlook.com"}[15m])) > 3
```

## Repeated delivery delays

```promql
sum by (from_domain) (increase(resend_email_events_total{event_type="email.delivery_delayed"}[30m])) >= 3
```
...
```promql
sum(increase(resend_webhook_signature_failures_total[15m])) > 0
```

Exporter has stopped receiving events it used to receive (dead webhook config, broken ingress):

```promql
time() - max(resend_webhook_last_event_timestamp_seconds{event_type="email.delivered"}) > 86400
```

Tune the threshold to your sending volume — a low-volume sender may legitimately be quiet for a day. The timestamp is initialized to process start, so this alert works even right after a restart (a restart resets the staleness clock by design).
